...
sub handleShow {
    ...
    return inlineError( 'ERROR: unknown queue ' . $queue )
      unless $queue =~ /^(in|out|cur|trash)$/;
...
sub afterSaveHandler {
    ...
    if ( $web =~ /^_/ ) {
        writeDebug(
            "bailing out afterSaveHandler ... no pings for template webs");
        return;
    }
...
# in the preference cache yet; this SMELLs
    ...
    my $enablePingbackRegex =
      qr/^${setRegex}ENABLEPINGBACK\s*=\s*(on|yes|1|off|no|0)$/o;
    ...
    $isEnabled = Foswiki::Plugins::PingBackPlugin::isPingBackEnabled
      unless $found;
    if ($isEnabled) {
        ...
        writeDebug(
"bailing out afterSaveHandler ... not generating pingbacks for $web.$topic"
        );
        return;    # nop
    }
