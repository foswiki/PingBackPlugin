...
sub getClient {
    ...
    $pingClient = new Foswiki::Plugins::PingBackPlugin::Client()
      unless $pingClient;
...
sub detectServer {
    ...
    # check html header
        ...
        m/<link\s+rel=["']pingback['"]\s+href=["']([^"']+)["']\s*\/?>/
        || $content =~
        m/<link\s+href=["']([^"']+)["']\s+rel=["']pingback["']\s*\/?>/ )
    {
...
sub ping {
    ...
    if ( not ref $response ) {
        $status = 502;
        $result =
"Bad Gateway (not a valid XML-RPC response) from '$server':\n$response\n";
    }
